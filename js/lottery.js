...
var $popover = $('.popover'), $lottery = $('#lotterys'), $go = $('#go'), $modal = $('.popover,.modal'), $lottery_num = $('#lottery_num'), total_num = getCookie('LOTTERY_TOTAL_NUM') || 1; //抽奖次数
	...
	$lottery.rotate({
		...
		callback: function (){
			...
			if(item == 3 || item == 7){
                ...
                setCookie('record', txt, 96);
				record_log(getCookie('record')); //插入我的中奖纪录
			}
			changeNum(total_num);
			_lottery.isLock = !_lottery.isLock;
		}
...
function record_log(txt){
	...
	if(txt=='500现金券')txt='500现金券(满1500可用)';
	tpl = (txt != '') ? '<li><p style="font-size:20px">'+txt+'<span>x 1</span></p></li>' : '<li class="empty_record"><p>暂无记录</p></li>';
...
$(function(){
    ...
    //初始化我的抽奖记录
    record_log(getCookie('record'));
    
    //初始化抽奖次数
    changeNum(total_num);
    
	//动态添加大转盘的奖品与奖品区域背景颜色
	_lottery.title = ["500现金券", "300现金券", "200现金券", "谢谢参与", "100现金券", "谢谢参与", "50现金券 ", "谢谢参与"];
